...
reset role;
insert into ita_reports(id,source_post_id,source_page,source_url,published_at,content_hash,body,raw,extraction_attempts,extraction_error,next_extraction_at)
values('17000000-0000-4000-8000-000000000007','pending-retry','traficalg','https://www.facebook.com/traficalg',now(),repeat('b',64),'fixture','{}',3,'quote mismatch',now()+interval '1 hour');
set local role authenticated;
select lives_ok($$select public.retry_ita_report('17000000-0000-4000-8000-000000000007')$$,'admin expedites pending failed report');
select is((select extraction_attempts from ita_reports where id='17000000-0000-4000-8000-000000000007'),3,'pending retry preserves attempt history');
select is((select extraction_error from ita_reports where id='17000000-0000-4000-8000-000000000007'),'quote mismatch','pending retry preserves failure evidence');
select ok((select next_extraction_at<=clock_timestamp() from ita_reports where id='17000000-0000-4000-8000-000000000007'),'pending retry removes backoff');
select throws_ok($$select public.retry_ita_report('17000000-0000-4000-8000-000000000007')$$,'55000','ita_retry_already_queued','double click cannot queue repeated work');
select is((select count(*) from admin_audit where action='ita.retry' and target_id='17000000-0000-4000-8000-000000000007'),1::bigint,'pending retry is audited once');
reset role;
select ok(exists(select 1 from source_jobs where contract_key='ita_website' and trigger_kind='manual' and state='queued' and available_at<=clock_timestamp()),'manual retry queues runnable source work');
update ita_reports set extraction='{}' where id='17000000-0000-4000-8000-000000000007';
set local role authenticated;
select throws_ok($$select public.retry_ita_report('17000000-0000-4000-8000-000000000007')$$,'55000','ita_report_not_retryable','completed report cannot be retried');
reset role;
update ita_reports set extraction=null,extraction_attempts=0,extraction_error=null where id='17000000-0000-4000-8000-000000000007';
set local role authenticated;
select throws_ok($$select public.retry_ita_report('17000000-0000-4000-8000-000000000007')$$,'55000','ita_report_not_retryable','unattempted report cannot bypass queue');
reset role;
update ita_reports set extraction_attempts=3,extraction_error='quote mismatch',next_extraction_at=now()+interval '1 hour' where id='17000000-0000-4000-8000-000000000007';
insert into source_job_leases(contract_key,job_id,worker_id,attempt,leased_at,lease_expires_at)
select 'ita_website',id,'manual-retry-test',1,now(),now()+interval '1 minute' from source_jobs where contract_key='ita_website' and trigger_kind='manual' limit 1;
set local role authenticated;
select throws_ok($$select public.retry_ita_report('17000000-0000-4000-8000-000000000007')$$,'55000','ita_extraction_in_progress','active source worker blocks manual retry');
reset role;
delete from source_job_leases where contract_key='ita_website' and worker_id='manual-retry-test';
update source_contracts set enabled=false where key='ita_website';
set local role authenticated;
select throws_ok($$select public.retry_ita_report('17000000-0000-4000-8000-000000000007')$$,'55000','ita_source_paused','manual retry respects source pause');
select * from finish();
rollback;
