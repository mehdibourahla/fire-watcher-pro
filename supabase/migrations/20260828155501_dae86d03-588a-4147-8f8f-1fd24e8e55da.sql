...
SET
  nearest_settlement_id = CASE WHEN ns.distance_km <= 15 THEN ns.settlement_id ELSE NULL END,
  nearest_settlement_km = CASE WHEN ns.distance_km <= 15 THEN round(ns.distance_km::numeric, 1)::double precision ELSE NULL END,
  commune_id = CASE WHEN nc.distance_km <= 15 THEN nc.commune_id ELSE NULL END,
  wilaya_id = CASE WHEN nc.distance_km <= 15 THEN nc.wilaya_id ELSE NULL END,
  updated_at = now()
