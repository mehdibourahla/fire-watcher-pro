...
psql() {
  if [ "${QA_CONNINFO_STUB:-}" = "host-address" ] && [[ "$*" == *"\\conninfo"* ]]; then
    printf '%s\n' \
      'Database|postgres' \
      'Host|127.0.0.1' \
...
assert_env_rejected() {
  local variable="$1" value="$2" output status
  set +e
  output="$(
    env "$variable=$value" \
      QA_DATABASE_URL="$local_db" \
      QA_REST_URL="http://127.0.0.1:54821" \
  ...
  test "$status" -eq 2
  case "$output" in
    *"refusing database environment overrides"*) ;;
    *) echo "missing environment override refusal for $variable" >&2; exit 1 ;;
  esac
  case "$output" in
    *"unexpected psql call"* | *"unexpected curl call"*)
      echo "environment guard ran after an external request for $variable" >&2
      exit 1
      ;;
  esac
}

assert_connected_address_rejected() {
  local output status
  set +e
  output="$(
    QA_CONNINFO_STUB="host-address" \
      QA_DATABASE_URL="$local_db" \
      QA_REST_URL="http://127.0.0.1:54821" \
      QA_ANON_KEY="local-anon" \
      QA_SERVICE_ROLE_KEY="local-service" \
  ...
  test "$status" -eq 2
  case "$output" in
    *"refusing unexpected connected database server"*) ;;
    *) echo "missing connected host-address refusal" >&2; exit 1 ;;
  esac
  case "$output" in
    *"unexpected psql call"* | *"unexpected curl call"*)
      echo "connected-address guard proceeded after verification" >&2
      exit 1
      ;;
  esac
}
...
assert_env_rejected PGHOSTADDR 203.0.113.9
assert_env_rejected PGHOST db.example.test
assert_env_rejected PGSERVICE production
assert_connected_address_rejected
