...
function ModerationPage() {
  ...
  const [tab, setTab] = useState<"reports" | "ideas" | "translations">(
    "reports",
  );
  ...
  return (
    <main className="mx-auto w-full max-w-4xl px-4 py-8">
      ...
      {tab === "translations" ? <TranslationQueue locale={locale} /> : null}
...
function TranslationQueue({ locale }: { locale: Locale }) {
  const { t } = useTranslation();
  const qc = useQueryClient();
  ...
  const filters: { key: SuggestionStatus; label: string }[] = [
    ...
    { key: "accepted", label: t("admin.filterAccepted") },
