...
Cloudflare contains the correct Resend DKIM and send-subdomain SPF/MX records. Public resolvers return them, but Resend remains Pending and its sending-only key selector offers only the existing verified domain. The Nadhir key, SMTP configuration and real delivery test must wait for verification. Do not reuse another application's credentials. PR #132 remains draft until operational setup is complete.
